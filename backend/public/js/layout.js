        ...
        const navItems = {
            admin: [
                { section: 'القائمة الرئيسية', items: [
                    { label: 'لوحة التحكم', icon: 'fa-home', url: '/admin/dashboard' },
                    { label: 'المستخدمين', icon: 'fa-users', url: '/admin/users' },
                    { label: 'البائعون المعلقون', icon: 'fa-user-clock', url: '/admin/sellers', badgeId: 'pending-badge' }
                ]},
                { section: 'المحتوى', items: [
                    { label: 'المنتجات', icon: 'fa-box', url: '/admin/products' },
                    { label: 'المزادات', icon: 'fa-gavel', url: '/admin/auctions' },
                    { label: 'الطلبات', icon: 'fa-shopping-cart', url: '/admin/orders' },
                    { label: 'الفئات', icon: 'fa-tags', url: '/admin/categories' }
                ]},
                { section: 'الإعدادات', items: [
                    { label: 'إعدادات النظام', icon: 'fa-cog', url: '/admin/settings' },
                    { label: 'العودة للموقع', icon: 'fa-arrow-right', url: '/' }
                ]}
            ],
            seller: [
                { section: 'القائمة الرئيسية', items: [
                    { label: 'لوحة التحكم', icon: 'fa-home', url: '/seller/dashboard' }
                ]},
                { section: 'المنتجات والمزادات', items: [
                    { label: 'منتجاتي', icon: 'fa-box', url: '/seller/products' },
                    { label: 'مزاداتي', icon: 'fa-gavel', url: '/seller/auctions' }
                ]},
                { section: 'إضافة جديد', items: [
                    { label: 'إضافة منتج', icon: 'fa-plus-circle', url: '/seller/add-product' },
                    { label: 'إنشاء مزاد', icon: 'fa-hammer', url: '/seller/create-auction' }
                ]},
                { section: 'المبيعات', items: [
                    { label: 'الطلبات', icon: 'fa-shopping-cart', url: '/seller/orders' },
                    { label: 'الإحصائيات', icon: 'fa-chart-line', url: '/seller/statistics' },
                    { label: 'الأرباح', icon: 'fa-dollar-sign', url: '/seller/earnings' }
                ]},
                { section: 'الإعدادات', items: [
                    { label: 'إعدادات الملف الشخصي', icon: 'fa-user-cog', url: '/seller/settings' },
                    { label: 'العودة للموقع', icon: 'fa-arrow-right', url: '/' }
                ]}
            ]
        };
