...
<script>
    ...
    function displayProducts(data) {
        ...
        tbody.innerHTML = products.map(product => `
            <tr class="border-b border-color hover:bg-tertiary hover:bg-opacity-50 transition">
                ...
                <td class="px-6 py-4">
                    <span class="px-3 py-1 bg-blue-500 bg-opacity-20 text-blue-500 rounded-full text-sm">
                ...
                <td class="px-6 py-4">
                    <span class="font-bold text-green-500">${product.price} ر.س</span>
                </td>
                <td class="px-6 py-4">
                    <span class="text-sm ${product.stock > 10 ? 'text-green-500' : product.stock > 0 ? 'text-yellow-500' : 'text-red-500'}">
                        ${product.stock}
                    </span>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                    <div class="flex gap-2">
                        <button onclick="toggleProductStatus(${product.id})" 
                            class="text-sm px-3 py-1 rounded ${product.is_active ? 'bg-red-500 text-red-500' : 'bg-green-500 text-green-500'} bg-opacity-20 hover:bg-opacity-30 transition"
    ...
    async function toggleProductStatus(id) {
        try {
            ...
            ui.showSuccess('تم تغيير حالة المنتج!');
